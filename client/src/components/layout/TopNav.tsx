...
const routeContexts: RouteContext[] = [
  { key: '/dashboard', title: '系统概览', description: '从首页进入协作、媒体与系统模块，并快速查看当前服务健康状态。', match: (pathname) => pathname === '/dashboard' },
  ...
  { key: '/friends', title: '好友关系', description: '管理联系人、邀请关系和私聊入口。', match: (pathname) => pathname.startsWith('/friends') },
  { key: '/docker', title: 'Docker 管理', description: '查看容器、执行操作并区分可控与受限资源。', match: (pathname) => pathname.startsWith('/docker') },
  ...
  { key: '/status', title: '系统状态', description: '查看服务与资源状态，快速定位异常节点。', match: (pathname) => pathname.startsWith('/status') },
  { key: '/settings', title: '个人设置', description: '管理当前账号的基础资料、偏好和安全设置。', match: (pathname) => pathname.startsWith('/settings') },
]
    ...
    <>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? 10 : 20, minWidth: 0 }}>
          ...
          >
            <span style={{ color: colors.primary, fontWeight: 700, fontSize: isMobile ? 18 : 20, lineHeight: 1.1, letterSpacing: -0.5 }}>
              CloudNexus
            </span>
            <Text style={{ fontSize: 11, color: colors.textSecondary }}>协作 · 媒体 · 系统</Text>
          </div>

          {!isMobile && (
            <>
              <div style={{ width: 1, height: 32, background: colors.borderSubtle, flexShrink: 0 }} />
              {/* 桌面端不铺全局导航按钮，改用当前模块上下文提示来保持顶栏克制。 */}
              <div style={{ minWidth: 0 }}>
                <div style={{ fontSize: 16, fontWeight: 600, color: colors.text }}>{currentContext.title}</div>
                <Text style={{ fontSize: 12, color: colors.textSecondary }} ellipsis>{currentContext.description}</Text>
              </div>
            </>
          )}

          {isMobile && (
            <Text style={{ fontSize: 13, fontWeight: 600, color: colors.text, maxWidth: 120 }} ellipsis>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
          <Button
            ...
            style={{ color: colors.textSecondary, fontSize: 18, borderRadius: radius.md }}
          />
      ...
      >
        <div style={{ padding: '0 16px 16px', borderBottom: `1px solid ${colors.borderSubtle}`, marginBottom: 8 }}>
        ...
        <Menu
          ...
          items={navItems.map((item) => ({ key: item.path, icon: item.icon, label: item.label }))}
