...
自托管、数据私有的协作平台 — 云存储、即时通讯、Docker 管理、视频监控、在线文档、AI 短剧工坊。
...
> **v0.1.0 已发布** — 全栈Docker化。v0.2.0 开发中：前端重构、相册、音乐、AI 短剧工坊、连续摄像头录制等；后续规划 WebDAV、消息搜索、FCM 推送等功能。
...
| 视频监控 | 摄像头管理、RTSP→HLS 实时播放、连续录制与回放、AI 目标检测、视频文件分析、人脸识别、考勤签到 | camera-svc |
| 在线文档 | 协作编辑、Markdown 增强、代码高亮、任务列表、实时同步 | collab-svc |
| AI 短剧工坊 | 剧本解析、分镜/片段/资产管理、AI 图片与视频生成（ComfyUI）、任务队列与进度推送 | drama-svc |
...
```
                   ...
                   /    |    |      \        \        \
     +-----------+ +-------+ +----------+ +----------+ +----------+ +----------+
     |user-file-svc| |im-svc| |docker-svc| |camera-svc| |collab-svc| |drama-svc |
     |  (:8081)   | |(:8082)| | (:8083)  | | (:8085)  | | (:8086)  | | (:8087)  |
     +-----------+ +-------+ +----------+ +----------+ +----------+ +----------+
          |    |       |    |      |             |            |            |
          v    v       v    v      v             v            v            v
     +--------+ +-------+ +----+ +===========+ +----------+ +----------+ +=========+
     |PostgreSQL| | MinIO | |Redis| |Docker   | | MediaMTX | |AI        | |ComfyUI  |
     | (:5432) | |(:9000)| |:6379| |Daemon   | | (:8888)  | |Inference | | (:8188) |
     +--------+ +-------+ +----+ |(socket) | +----------+ |(:8000)   | +=========+
                                 +===========+              +----------+
...
curl http://localhost/healthz/collab-svc   | jq
curl http://localhost/healthz/drama-svc    | jq
...
curl http://localhost:8087/healthz  # DB + Redis 状态
```
...
| [AI 短剧工坊](docs/drama.md) | drama-svc 模块边界、生成流程、API |
| [短剧生成流程](docs/drama-generation-flow.md) | 剧本 → 分镜 → 素材 → 生成的完整流程 |
...
├── server/          Go 多服务后端
│   ├── cmd/         服务入口 (user-file-svc, im-svc, docker-svc, camera-svc, collab-svc, drama-svc)
│   ├── internal/    每服务独立逻辑 (handler → service → repository)
│   ├── pkg/         跨服务共享 (auth, middleware, database, cache, system...)
│   └── config/      YAML 配置 (单机/Docker/集群)
├── deploy/          Docker Compose、Nginx 配置、AI 推理、ComfyUI、流媒体
├── docs/            项目文档
└── scripts/         开发脚本
```
...
| drama-svc | 8087 | Gin + Redis 队列 + ComfyUI | AI 短剧工坊：剧本解析、分镜/资产、图片/视频生成任务 |
...
| AI 检测 | YOLOv8 (Python FastAPI) |
| AI 生成 | ComfyUI (SDXL / Wan2.2, Redis 异步任务队列) |
...
| Phase 4 | 摄像头AI + 在线文档 + 版本管理 + 安全增强 | ✅ |
| Phase 5 | 前端重构 + 相册 + 音乐 + 服务状态 | ✅ |
| Phase 6 | 前端模块化重构 + 后端工具链完善 | ✅ |
| Phase 7 | AI 短剧工坊 (drama-svc + ComfyUI) | 🔄 |
| Phase 8 | 高级功能 (WebDAV/消息搜索/OAuth/K8s/性能调优) | ⬜ |
